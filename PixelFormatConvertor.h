 ...
 * Header file for the CPixelFormatConvertor class
 *
 * Conversion functions between pixel formats.
 *
...
#if !defined(AFX_PIXELFORMATCONVERTOR_H__38140580_FD21_11D2_96B4_00A0C982B57D__INCLUDED_)
...
class DLL_EXP CPixelFormatConvertor  
{
public:
	CPixelFormatConvertor();
	virtual ~CPixelFormatConvertor();

	int Conv24to16(BYTE *pSrc, int width, int height, BYTE *pDst);
	int Conv15to16(BYTE *pSrc, int width, int height);
	int ConvPelFormat(BYTE *pData, int width, int height, int RedMask, int GreenMask, int BlueMask);
};

#endif // !defined(AFX_PIXELFORMATCONVERTOR_H__38140580_FD21_11D2_96B4_00A0C982B57D__INCLUDED_)
